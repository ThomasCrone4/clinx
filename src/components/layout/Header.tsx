...
import { getAllAlerts, getAlertsForTeacher } from '../../services/dataService';

export default function Header() {
  const { user, logout } = useAuth();
  const navigate = useNavigate();

  const alerts = user?.role === 'teacher'
    ? getAlertsForTeacher(user.name).filter(a => a.status === 'Unread')
    : getAllAlerts().filter(a => a.status === 'Unread');
