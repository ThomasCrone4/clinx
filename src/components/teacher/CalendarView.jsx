...
export default function CalendarView() {
  ...
  if (!teacher) return <p className="text-gray-500">Teacher data not available</p>;
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              ...
              {days.map(day => (
                <th key={day} className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase text-left">{day}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {periods.map((period, pi) => (
              <tr key={period.id}>
                ...
                {days.map(day => {
                  const slot = teacher.timetable[day]?.[pi];
                  if (!slot) return <td key={day} className="px-2 py-1"><div className="h-16" /></td>;
